...
void SDLCreateBackgroundHostMenu(SDL_Renderer *mainRenderer, SDLBackground *background){
    ...
    if((background->texture=SDL_CreateTexture(mainRenderer, SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_STREAMING,background->rect.w,background->rect.h))!=NULL){
        
        SDLAnimateBackgroundHostMenu( mainRenderer,background->texture, background->color );
